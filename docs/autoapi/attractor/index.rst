...
.. autoapisummary::
   ...
   attractor.exp_attractor
   attractor.inv_attractor



.. py:function:: exp_attractor(dx, alpha: float = 300, gamma: int = 2)
   ...
   :param dx: The difference tensor dx = Ai - Cj, where Ai is the attractor point and Cj is the bin center.
   :type dx: torch.Tensor
   :param alpha: Proportional Attractor strength. Determines the absolute strength. Lower alpha = greater attraction. Defaults to 300.
   :type alpha: float, optional
   :param gamma: Exponential Attractor strength. Determines the "region of influence" and indirectly number of bin centers affected. Lower gamma = farther reach. Defaults to 2.
   :type gamma: int, optional

   :returns: Delta shifts - dc; New bin centers = Old bin centers + dc
   :rtype: torch.Tensor


.. py:function:: inv_attractor(dx, alpha: float = 300, gamma: int = 2)
...
.. py:class:: AttractorLayer(in_features, n_bins, n_attractors=16, mlp_dim=128, min_depth=0.001, max_depth=10, alpha=300, gamma=2, kind='sum', attractor_type='exp', memory_efficient=False)
   ...
   .. py:method:: forward(x, b_prev, prev_b_embedding=None, interpolate=True, is_for_query=False)

      :param x: feature block; shape - n, c, h, w
      :type x: torch.Tensor
      :param b_prev: previous bin centers normed; shape - n, prev_nbins, h, w
      :type b_prev: torch.Tensor

      :returns: new bin centers normed and scaled; shape - n, nbins, h, w
...
.. py:class:: AttractorLayerUnnormed(in_features, n_bins, n_attractors=16, mlp_dim=128, min_depth=0.001, max_depth=10, alpha=300, gamma=2, kind='sum', attractor_type='exp', memory_efficient=False)
   ...
   .. py:method:: forward(x, b_prev, prev_b_embedding=None, interpolate=True, is_for_query=False)
      ...
      :returns: new bin centers unbounded; shape - n, nbins, h, w. Two outputs just to keep the API consistent with the normed version
      :rtype: tuple(torch.Tensor,torch.Tensor)
