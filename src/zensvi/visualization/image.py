...
def clean_pattern(pattern, image_extensions):
...
def plot_image(dir_image_input: Union[str, Path],
                         n_row: int, 
                         n_col: int, 
                         subplot_width: int = 3,
    ...
    if image_file_pattern is not None:
        ...
        image_files = [file for ext in image_extensions for file in dir_image_input.rglob(f"*.{ext}")]
    ...
    if dir_csv_input is not None and csv_file_pattern is not None:
        ...
        if image_file_pattern is not None:
            image_file_pattern_no_regex = clean_pattern(image_file_pattern, image_extensions)
        ...
        if not use_all:
            ...
            if n_row * n_col > len(df_filtered):
                raise ValueError(f"n_row * n_col ({n_row * n_col}) is greater than the number of images ({len(df_filtered)})")
        ...
        if sort_by.lower() != 'random':
            ...
            df_filtered = df_filtered.sample(frac=1, random_state=random_seed).reset_index(drop=True)  # Randomly shuffle the DataFrame
    ...
    # set title
    prop_title, _, _ = get_font_properties(font_size)
