...
def _detect_blob_single_image(image_path, dir_image_output=None):
    ...
    image_with_blobs = cv2.drawKeypoints(image, keypoints, None, (0, 0, 255), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
...
def _calculate_hsl_single_image(image_path, dir_image_output=None):
    ...
    return {'avg_hue': avg_hue, 'avg_saturation': avg_saturation, 'avg_lightness': avg_lightness}
...
def get_low_level_features(dir_input, 
    ...
    images = [file for ext in image_extensions for file in Path(dir_input).rglob(f"*{ext}")] if Path(dir_input).is_dir() else [Path(dir_input)]
    
    summary_data = []
    with ThreadPoolExecutor() as executor:
        future_to_image = {executor.submit(_detect_all_features_single_image, image, dir_image_output): image for image in images}
