...
from zensvi.download_async.utils.imtool import ImageTool
from zensvi.download_async.utils.get_pids import panoids
from zensvi.download_async.utils.geoprocess import GeoProcessor
from zensvi.download_async.utils.helpers import standardize_column_names, create_buffer_gdf
...
class StreetViewDownloader:
    ...
    def _get_ua(self):
        user_agent_file = pkg_resources.resource_filename('zensvi.download_async.utils', 'UserAgent.csv')
        UA = []
    ...
    def _get_proxies(self):
        ...
        with open(proxies_file, 'r') as f:
            ...
            for row in reader:
                ...
                proxies.append(f"{list(proxy_dict.keys())[0]}://{list(proxy_dict.values())[0]}")
        return proxies
    ...
    async def _get_pids_from_csv(self, df, id_columns, closest=False, disp=False):
        scheduler = await aiojobs.create_scheduler()

        async def get_street_view_info(longitude, latitude, proxies):
            results = await panoids(latitude, longitude, proxies, closest=closest, disp=disp)
            return results

        async def worker(row):
            input_longitude = row.longitude
            input_latitude = row.latitude
            id_dict = {column: getattr(row, column) for column in id_columns}
            row_results = await get_street_view_info(input_longitude, input_latitude, self.proxies)
        ...
        tasks = [worker(row) for row in df.itertuples()]
        
        results = []
        for f in tqdm_async(asyncio.as_completed(tasks), total=len(tasks), desc="Getting pids"):
            result = await f
    ...
    def _get_pids_from_gdf(self, gdf, id_columns, closest=False, disp=False):  
        ...
        results_df = asyncio.run(self._get_pids_from_csv(df, id_columns, closest=closest, disp=disp))
    ...
    def get_pids(self, path_pid, lat = None, lon = None, input_csv_file = "", input_shp_file = "", id_columns=None, buffer = 0, closest=False, disp=False, augment_metadata=False):
        ...
        if lat != None and lon != None:
            ...
            if buffer > 0:
                ...
                pid = asyncio.run(self._get_pids_from_csv(df, id_columns, closest=closest, disp=disp))
        elif input_shp_file != "":
