...
def output_dir(base_output_dir, ensure_dir):
    output_dir = base_output_dir / "lighting"
...
def test_classify_directory(output_dir, input_dir, all_devices):
    ...
    dir_summary_output = str(Path(output_dir) / f"{all_devices}/directory/summary")
    classifier.classify(
        image_input,
...
def test_classify_single_image(output_dir, input_dir, all_devices):
    ...
    dir_summary_output = str(Path(output_dir) / f"{all_devices}/single/summary")
    classifier.classify(
        image_input,
        dir_summary_output=dir_summary_output,
    )
