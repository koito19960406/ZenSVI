...
def output(base_output_dir, ensure_dir):
    output_dir = base_output_dir / "classification/lighting"
    ensure_dir(output_dir)
    return output_dir


def test_classify_directory(output, input_dir, all_devices):
    classifier = ClassifierLighting(device=all_devices)
    image_input = str(input_dir / "images")
    dir_summary_output = str(Path(output) / f"{all_devices}/directory/summary")
    classifier.classify(
        image_input,
...
def test_classify_single_image(output, input_dir, all_devices):
    ...
    dir_summary_output = str(Path(output) / f"{all_devices}/single/summary")
    classifier.classify(
        image_input,
        dir_summary_output=dir_summary_output,
    )
