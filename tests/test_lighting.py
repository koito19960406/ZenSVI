...
def test_classify_directory(output_dir, input_dir, cpu_device):
    classifier = ClassifierLighting(device=cpu_device)
    image_input = str(input_dir / "images")
    dir_summary_output = str(Path(output_dir) / f"{cpu_device}/directory/summary")
    classifier.classify(
        image_input,
...
def test_classify_single_image(output_dir, input_dir, cpu_device):
    ...
    dir_summary_output = str(Path(output_dir) / f"{cpu_device}/single/summary")
    classifier.classify(
        image_input,
        dir_summary_output=dir_summary_output,
    )
