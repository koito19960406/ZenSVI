...
def test_verbosity_levels(output_dir, input_dir, all_devices):
    ...
    silent_classifier = ClassifierPerception(
        perception_study="more boring",
        device=all_devices,
        verbosity=0
    )
    ...
    default_classifier = ClassifierPerception(
        perception_study="more boring",
        device=all_devices
    )
    ...
    verbose_classifier = ClassifierPerception(
        perception_study="more boring",
        device=all_devices,
        verbosity=2
    )
    ...
    default_classifier.classify(
        image_input,
        dir_summary_output=str(output_dir / f"{all_devices}/perception_default_verbosity/summary")
    )
    ...
    # Test with verbosity=0 (no progress bars)
    silent_vit_classifier = ClassifierPerceptionViT(
        perception_study="more boring",
        device=all_devices,
        verbosity=0
    )
    ...
    # Test with default verbosity (1)
    default_vit_classifier = ClassifierPerceptionViT(
        perception_study="more boring",
        device=all_devices
    )
