...
def test_detect_objects_directory(output_dir, input_dir, cpu_device):
    detector = ObjectDetector(text_prompt="tree .", box_threshold=0.45, text_threshold=0.25, device=cpu_device)
    image_input = str(input_dir / "images")
    dir_image_output = str(output_dir / f"{cpu_device}/directory/images")
    summary_output = str(output_dir / f"{cpu_device}/directory/summary")
...
def test_detect_objects_single_image(output_dir, input_dir, cpu_device):
    detector = ObjectDetector(text_prompt="tree .", box_threshold=0.45, text_threshold=0.25, device=cpu_device)
    # Specify a single image file inside the images directory.
    image_input = str(input_dir / "images" / "-3vfS0_iiYVZKh_LEVlHew.jpg")
    dir_image_output = str(output_dir / f"{cpu_device}/single/images")
    summary_output = str(output_dir / f"{cpu_device}/single/summary")
    detector.detect_objects(
        dir_input=image_input, dir_image_output=dir_image_output, dir_summary_output=summary_output, max_workers=1
    )
...
def test_detect_objects_image_output_only(output_dir, input_dir, cpu_device):
    detector = ObjectDetector(text_prompt="tree .", box_threshold=0.45, text_threshold=0.25, device=cpu_device)
    image_input = str(input_dir / "images")
    dir_image_output = str(output_dir / f"{cpu_device}/images_only")
    detector.detect_objects(
...
def test_detect_objects_summary_output_only(output_dir, input_dir, cpu_device):
    detector = ObjectDetector(text_prompt="tree .", box_threshold=0.45, text_threshold=0.25, device=cpu_device)
    image_input = str(input_dir / "images")
    summary_output = str(output_dir / f"{cpu_device}/summary_only")
    detector.detect_objects(
...
def test_detect_objects_no_output_directories_error(output_dir, input_dir, cpu_device):
    detector = ObjectDetector(text_prompt="tree .", box_threshold=0.45, text_threshold=0.25, device=cpu_device)
    image_input = str(input_dir / "images")
...
def test_detect_objects_single_image_summary_only(output_dir, input_dir, cpu_device):
    detector = ObjectDetector(text_prompt="tree .", box_threshold=0.45, text_threshold=0.25, device=cpu_device)
    # Specify a single image file inside the images directory
    image_input = str(input_dir / "images" / "-3vfS0_iiYVZKh_LEVlHew.jpg")
    summary_output = str(output_dir / f"{cpu_device}/single_image_summary_only")
    detector.detect_objects(
        dir_input=image_input, dir_image_output=None, dir_summary_output=summary_output, max_workers=1
    )
    # Check that summary files were created
...
def test_detect_objects_grouped_summary(output_dir, input_dir, cpu_device):
    detector = ObjectDetector(text_prompt="tree .", box_threshold=0.45, text_threshold=0.25, device=cpu_device)
    image_input = str(input_dir / "images")
    summary_output = str(output_dir / f"{cpu_device}/grouped_summary")
    detector.detect_objects(
    ...
    summary_output_csv = str(output_dir / f"{cpu_device}/grouped_summary_csv")
    detector.detect_objects(
        dir_input=image_input,
        dir_image_output=None,
