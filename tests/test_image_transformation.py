...
def test_verbosity_levels(output_dir, input_dir):
    ...
    default_transformer = ImageTransformer(
        dir_input, 
        str(output_dir / "default_verbosity"),
        log_path=output_dir / "default_verbosity.log"
    )
    ...
    silent_transformer = ImageTransformer(
        dir_input, 
        str(output_dir / "silent_verbosity"),
        log_path=output_dir / "silent_verbosity.log",
        verbosity=0
    )
    ...
    verbose_transformer = ImageTransformer(
        dir_input, 
        str(output_dir / "verbose_verbosity"),
        log_path=output_dir / "verbose_verbosity.log",
        verbosity=2
    )
    ...
    default_transformer.transform_images(
        style_list="perspective equidistant_fisheye"
    )
    
    # Run with verbosity set explicitly in method call (overriding instance verbosity)
    silent_transformer.transform_images(
        style_list="perspective equidistant_fisheye",
        verbosity=2
    )
